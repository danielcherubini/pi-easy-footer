    ...
    ctx.ui.setFooter((tui, theme, footerData) => {
      ...
      return {
        ...
        render(width: number): string[] {
          ...
          try {
            ...
            if (worktrees.length > 1) {
              ...
              for (const wt of worktrees) {
                ...
                if (pathLine && pathLine.replace("worktree ", "") === currentPath) {
                  if (branchLine) worktree = branchLine.replace("branch refs/heads/", "");
                  break;
                }
              }
            }
          ...
          const contextPctVal  = contextUsage?.percent ?? (contextWindow > 0 ? (totalInput + totalOutput) / contextWindow * 100 : 0);
          ...
          if (contextWindow > 0) {
            ...
            contextBar = slate(iconContext + " [") + barColor(bar) + slate("] " + Math.round(contextPctVal) + "%");
          }
          ...
          const thinkingStr = thinkingLevel !== "off"
            ? theme.fg(thinkingColors[thinkingLevel] || "dim", "◐ " + thinkingLevel)
            : "";
          ...
          const line1Right = [contextBar, thinkingStr].filter(Boolean).join(theme.fg("dim", "  "));
          const pad1  = " ".repeat(Math.max(1, width - visibleWidth(line1Left) - visibleWidth(line1Right)));
          ...
          // Context % with color
          const contextDisplay = contextPct === "?"
            ? "?/" + formatTokens(contextWindow)
            : contextPct + "%/" + formatTokens(contextWindow);
          const contextColored = contextPctVal > 90
